...
const shareBandwidth = async (token) => {
    ...
    while (attempt < maxRetries) {
        try {
            ...
            const profileResponse = await fetch('https://api.openloop.so/users/profile', {
                method: 'GET',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json',
            ...
            if (!profileResponse.ok) {
                throw new Error(`Failed to fetch user profile! Status: ${profileResponse.statusText}`);
            }

            const profileData = await profileResponse.json();
            const username = profileData?.data?.username || 'Unknown User';
            const name = profileData?.data?.name || 'Unknown User';

            const balanceRaw = data?.data?.balances?.POINT || 'N/A';
			const balance = parseFloat(balanceRaw).toFixed(2); 
            logger(
                `Bandwidth shared Message: ${chalk.yellow(data.message)} | Score: ${chalk.yellow(quality)} | Total Earnings: ${chalk.yellow(balance)} | Name: ${chalk.cyan(name)} | Username: ${chalk.cyan(username)}`
            );
...
const main = () => {
    ...
    intervalId = setInterval(shareBandwidthForAllTokens, 240 * 1000);
};
main();
